...
{
    public sealed class FindStructByNameVisitor<T> : AbstractTypeSearchVisitor<T>
        where T : IStruct
    {
        public FindStructByNameVisitor(string name)
        {
            Name = name;
        }
        ...
        {
            VisitChildren(loadedDocument.Structs, loadedDocument.Namespaces);
        }
        ...
        {
            VisitChildren(@namespace.Structs);
        }
    }
}
